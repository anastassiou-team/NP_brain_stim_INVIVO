...
def analyze_all_areas_mmr(
    ...
    for area in brain_areas:
        pre_data, stim_data, full_data = load_paired_condition_data_mmr(
...
def sort_areas_by_distance_mmr(results):
    """
...
def identify_entrained_areas_mmr(results, thresh_low=0.01, thresh_high=0.001, use_corrected=True):
    """
...
def prepare_spatial_decay_data_mmr(
    brain_areas,
    amplitude,
    frequency,
    frequencies_all,
    ...
    animal_list : list
        Animal IDs to include
    use_absolute : bool
        If True, use absolute difference |VL_stim - MMR_Pre|
        If False, use signed difference (VL_stim - MMR_Pre)
...
def get_entrained_area_indices_mmr(areas_with_data, entrained_areas):
    """
