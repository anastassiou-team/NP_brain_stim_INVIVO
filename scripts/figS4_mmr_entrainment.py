...
uses "MMR" (lowercase) instead of "MMR" for y-axis labels.
"""
import sys
import os
...
from src.data_loading_mmr import load_paired_condition_data_mmr
from src.analysis_core_mmr import (
    analyze_all_areas_mmr, 
    identify_entrained_areas_mmr,
    prepare_spatial_decay_data_mmr, 
    get_entrained_area_indices_mmr,
    sort_areas_by_distance_mmr,
...
def plot_spatial_decay_twopanel(
    ...
    if show_ylabel:
        ax_scatter.set_ylabel('|ΔMMR|' if SPATIAL_DECAY_OPTIONS['use_absolute_difference'] else 'ΔMMR', 
                              fontsize=8)
...
def prepare_regression_data_with_connectivity(amplitude, frequency, animal_list=None):
    ...
    area_results = analyze_all_areas_mmr(
    ...
    for area, data in area_results.items():
        ...
        _, _, full_data = load_paired_condition_data_mmr(
...
def generate_complete_figure2(save_figures=True):
    ...
    for row_idx, (amplitude, frequency) in enumerate(FIGURE2_PARAMS):
        ...
        sorted_areas, sorted_distances = sort_areas_by_distance_mmr(results)
        ...
        entrained = identify_entrained_areas_mmr(results, thresh_low=0.01, thresh_high=0.001, use_corrected=True)
    ...
    for idx, (amplitude, frequency) in enumerate(FIGURE2_SPATIAL_PARAMS):
        ...
        # impose multiple comparison testing..
        ...
        entrained = identify_entrained_areas_mmr(
            area_results,
        ...
        spatial_data = prepare_spatial_decay_data_mmr(
            ALL_BRAIN_AREAS, amplitude, frequency, FREQUENCIES,
            DEFAULT_ANIMAL_LIST,
        ...
        entrained_indices = get_entrained_area_indices_mmr(
